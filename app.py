...
with st.sidebar:
    ...
    auto_recognize = st.checkbox("入库时自动识别底盘", value=True)
    ...
    if all_cars:
        df = pd.DataFrame(all_cars, columns=['ID', '品牌', '车型', '颜色', '系列', '备注', '侧视图', '底盘图', '入库时间'])
        st.write("品牌分布：")
        st.bar_chart(df['品牌'].value_counts())

st.title("🚗 小车收藏管理")
st.write("侧视图+底盘图录入，自动识别车型，防重复购买")
...
with tab1:
    st.header("上传车辆照片")
    st.write("需要上传两张图片：侧视图（看颜色）+ 底盘图（看车名）")
        ...
        with col1:
            if st.button("✅ 确认入库", type="primary"):
                if not model or not color:
                    st.error("车型名称和颜色为必填项")
                else:
                    db.add_car(brand, model, color, series, note, 
    ...
    if all_cars:
        ...
        for idx, row in df.iterrows():
            with st.container():
                ...
                with col3:
                    st.write(f"**{row['车型']}**")
                    st.write(f"品牌：{row['品牌']} | 颜色：{row['颜色']}")
                    if row['系列']:
                        st.write(f"系列：{row['系列']}")
                    if row['备注']:
...
st.divider()
st.caption("💡 保存API Key → 上传侧视图+底盘图 → 确认车型和颜色 → 入库")
